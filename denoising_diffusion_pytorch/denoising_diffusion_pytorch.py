...
class MyTrainer:
    def __init__(
        ...
        *,
        ...
        self.ds = MyDataset(dataset_data)
    ...
    def train(self):
        ...
        with tqdm(
            ...
            while self.step < self.train_num_steps:
                ...
                accelerator.wait_for_everyone()
                self.step += 1
                # print("current training step is:", self.step)
                if accelerator.is_main_process:
                    self.ema.update()
                    ...
                    if self.step != 0 and divisible_by(
                        ...
                        print("saveing and resampling in current step", self.step)
                        self.ema.ema_model.eval()

                        with torch.inference_mode():
                            print("torch inference model is activated")
                            milestone = self.step // self.save_and_sample_every
                            batches = num_to_groups(self.num_samples, self.batch_size)
                            ...
                            print("all_images_list is completeted")

                        all_images = torch.cat(all_images_list, dim=0)

                        print("saving all_images_list to file")
                        utils.save_image(
                            all_images,
                            str(self.results_folder / f"sample-{milestone}.png"),
                            nrow=int(math.sqrt(self.num_samples)),
                        )
                        print("saving all_images_list to file done")

                        # whether to calculate fid

                        if self.calculate_fid:
                            print("self.calculate_fid is activated")
                            fid_score = self.fid_scorer.fid_score()
                            accelerator.print(f"fid_score: {fid_score}")
                        if self.save_best_and_latest_only:
                            print("self.save_best_and_latest_only is activated")
                            if self.best_fid > fid_score:
                                self.best_fid = fid_score
                                self.save("best")
                            self.save("latest")
                            ...
                            print(
                                "self.save_best_and_latest_only is not activated and just save the milestone"
                            )
                            self.save(milestone)
                # print("update pbar")
                pbar.update(1)
                # print("update pbar done")
            # print("current time step", self.step,"finished"

        accelerator.print("training complete")

    def test(self, return_all_timesteps=False):
        with torch.inference_mode():
            print("torch inference model is activated")
            milestone = self.step // self.save_and_sample_every
            batches = num_to_groups(self.num_samples, self.batch_size)
            ...
            all_images_list = list(
                map(
                    lambda n: self.ema.ema_model.sample(
                        batch_size=n, return_all_timesteps=return_all_timesteps
                    ),
                    batches,
                )
            )
        ...
        if return_all_timesteps == False:
            print("saving all_images_list to file")
            utils.save_image(
                all_images,
                str(self.results_folder / f"test_sample-{milestone}.png"),
                nrow=int(math.sqrt(self.num_samples)),
            )
            print("saving all_images_list to file done")
        return all_images
...
class Trainer3D:
    ...
    def save(self, milestone):
        ...
        # # Generate a few samples for visual inspection
        # with torch.no_grad():
        #     self.ema.ema_model.eval()
        #     batches = num_to_groups(self.num_samples, self.batch_size)
        ...
        #         sample_np = sample_np.squeeze() * 1000 - 1000  # rescale from [0,1] -> [-1000,1000]
        #         save_path = self.results_folder / f"model-{milestone}-sample-{i}.nrrd"
        #         nrrd.write(str(save_path), sample_np.astype(np.float32))

        # -------inference for Fourier Diffusion Model-------
        with torch.no_grad():
            # self.ema.ema_model.eval()
            # batches = num_to_groups(self.num_samples, self.batch_size)
            # samples = [self.ema.ema_model.sample(batch_size=n) for n in batches]
            ...
            # for i, sample in enumerate(samples):
            #     sample_np = sample.cpu().numpy()
            #     sample_np = (sample_np.squeeze() + 1) / 2.0 * 255.0
            #     save_path = self.results_folder
            #     image = torch.from_numpy(sample_np).cuda().squeeze(0)  # [C, D, H, W]

            #     recover = RecoverFreq()
            #     # high_freq_resample = recover.compress(high_freq)
            #     hf_process_resample = HFreq.mag_phase_to_complex(image[:2])  # 64*64*64

            #     pred_low = upsample(image[2], target_size=(256, 256, 256))

            #     pred_high = recover.recover(hf_process_resample)  # [2, 256, 256, 256]
            #     reconstructed = reconstruct_from_high_low(pred_low, pred_high)

            #     # reconstructed = (reconstructed + 1.0) / 2.0 * 255.0  # [0, 255]
            #     reconstructed = reconstructed.cpu().numpy()
            #     image_np = self.ds[0][0].squeeze(0).cpu().numpy()  # [D, H, W]
            ...
            #     plt.axis("off")
            #     plt.tight_layout()
            #     plt.savefig(save_path / f"sample_{i}_step_{self.step}.png")
            #     plt.close()
            self.ema.ema_model.eval()
            batches = num_to_groups(self.num_samples, self.batch_size)
            samples = [
            ...
            for i, sample in enumerate(samples):
                ...
                torch.save(
                    image,
                    save_path / f"sample-{milestone}-{i}.pt",
                )
    ...
    def load(self, milestone):
        data = torch.load(
            str(self.results_folder / f"model-{milestone}.pt"), map_location=self.device
        )
        model = self.accelerator.unwrap_model(self.model)
        model.load_state_dict(data["model"])
        self.step = data["step"]
    ...
    #                 with self.accelerator.autocast():
    #                     loss = self.model(data) / self.gradient_accumulate_every
    #                     total_loss += loss.item()
    #                 self.accelerator.backward(loss)
    ...
    #                         torch.save(
    #                             sample.cpu(),
    #                             self.results_folder / f"sample-{milestone}-{i}.pt",
    #                         )

    #                     self.save(milestone)

    #                 if self.save_best_and_latest_only and total_loss < self.best_loss:
    ...
    def train(self):
        """version 2 for Fourier Diffusion Model"""
        with tqdm(
            initial=self.step,
            total=self.train_num_steps,
            disable=not self.accelerator.is_main_process,
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ct_img, data = next(self.dl)
                    ct_img = ct_img.to(self.device)
                    data = data.to(self.device)
                    
                    # assert data.shape[1] == 3 & data.shape[2] == 64 & data.shape[3] == 64 & data.shape[4] == 64, f"data shape should be [B, 3, D, H, W], not {data.shape}"
                    ...
                    #     vol = data_denorm[b, 0]  # shape [D, H, W]
                    ...
                    # x_high = torch.stack(
                    #     [
                    #         (x - x.min()) / (x.max() - x.min()) * 2 - 1
                    #         for x in outputs_high
                    #     ]
                    # )
                    ...
                    with self.accelerator.autocast():
                        loss = self.model(x_input) / self.gradient_accumulate_every
                        total_loss += loss.item()
                    self.accelerator.backward(loss)
    ...
    def test(self, return_all_timesteps=False):
        ...
        #         torch.save(
        #             sample.cpu(),
        #             self.results_folder / f"test-sample-{milestone}-{i}.pt",
        #         )
        return samples
